...
const STAT_CARDS = (stats: Stats) => [
  { label: 'Total Transaksi',  value: stats.totalCount.toLocaleString('id-ID'),    icon: ShoppingCart, color: 'text-indigo-600',  bg: 'bg-indigo-50' },
  { label: 'Total Pendapatan', value: formatRupiah(stats.totalRevenue),             icon: TrendingUp,   color: 'text-emerald-600', bg: 'bg-emerald-50' },
  { label: 'Rata-rata Order',  value: formatRupiah(stats.avgOrder),                 icon: Receipt,      color: 'text-amber-600',   bg: 'bg-amber-50' },
  { label: 'Selesai',          value: stats.completedCount.toLocaleString('id-ID'), icon: CreditCard,   color: 'text-purple-600',  bg: 'bg-purple-50' },
];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black text-gray-900 tracking-tighter">Transactions</h2>
          <p className="text-sm text-gray-500 mt-1 font-medium">
            {isSuperAdmin ? 'Semua transaksi dari seluruh cabang' : 'Transaksi cabang Anda'}
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
        ...
        <div className="flex items-center gap-2">
              ...
              <PopoverTrigger asChild>
                <button className={cn(
                  ...
                  <span className="max-w-[120px] truncate">
                    {branchFilter === 'ALL'
                      ? 'Semua Cabang'
                      : (branches.find((b) => b.id === branchFilter)?.name ?? 'Cabang')}
                  </span>
          ...
          <div className="flex items-center bg-white rounded-2xl shadow-sm p-1.5 gap-1 h-14">
            {([
              { v: 'desc',    label: 'Terbaru',   icon: ArrowDown },
              { v: 'asc',     label: 'Terlama',   icon: ArrowUp },
              { v: 'highest', label: 'Terbesar',  icon: TrendingUp },
              { v: 'lowest',  label: 'Terkecil',  icon: TrendingDown },
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  {['Invoice', 'Tanggal', ...(isSuperAdmin ? ['Cabang'] : []), 'Total', 'Status', 'Pelanggan', 'Items', ''].map((h) => (
          ...
          {selectedTx && (
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between bg-indigo-50 rounded-2xl px-5 py-4 border border-indigo-100">
                <span className="text-sm font-black text-indigo-700 uppercase tracking-wide">Total</span>
